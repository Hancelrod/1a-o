...
import { Camera, Heart, Infinity, Upload } from 'lucide-react';

const App: React.FC = () => {
  const [photos, setPhotos] = useState<Photo[]>([
    { id: '1', url: 'https://picsum.photos/600/600?random=1', caption: 'El inicio de nuestra aventura' },
    { id: '2', url: 'https://picsum.photos/600/800?random=2', caption: 'Momentos inolvidables' },
    { id: '3', url: 'https://picsum.photos/600/600?random=3', caption: 'Tú y yo, siempre' },
  ]);
  ...
  const handleFileUpload = (e: React.ChangeEvent<HTMLInputElement>) => {
    if (e.target.files && e.target.files[0]) {
      ...
      const newPhoto: Photo = {
        ...
        url: imageUrl,
        caption: 'Un nuevo recuerdo eterno...'
      };
      ...
      <section className="relative z-10 container mx-auto px-4 py-20">
        <div className="text-center mb-12">
          ...
          <p className="text-gray-400 font-light">
            Sube nuestras fotos favoritas para inmortalizar cada instante.
          </p>
        </div>
        ...
        <div className="flex justify-center mb-12">
           <label className="cursor-pointer group">
              ...
              <div className="flex items-center gap-3 bg-white/10 hover:bg-white/20 border border-white/20 px-8 py-4 rounded-full transition-all hover:scale-105 hover:neon-glow">
                ...
                <span className="font-future text-sm tracking-wide">AGREGAR FOTO</span>
              </div>
           </label>
        </div>
          ...
          {photos.map((photo) => (
            <div key={photo.id} className="group relative aspect-square rounded-xl overflow-hidden glass-panel transform transition-all hover:-translate-y-2 hover:shadow-[0_0_30px_rgba(236,72,153,0.4)]">
              ...
              <div className="absolute inset-0 bg-gradient-to-t from-black/80 via-transparent to-transparent opacity-0 group-hover:opacity-100 transition-opacity duration-300 flex flex-col justify-end p-6">
                <p className="text-white font-love text-2xl">{photo.caption}</p>
